...
/*
  ...
  Format like "25.00zł";
*/
const sumOfMediumProducts: any = someArray.reduce((sum, product) => {
    if (product.size == "M") {
      return (sum + product.price)/100;
    }
    return sum/100;}, 0).toFixed(2) + "zł";
...
*/
const numberOfProductsWithReduce: any = someArray.reduce((sum, product)=>{
  if(product.size == "S"){
    return sum + 1;
  }
  else{
...
const numberOfProductsWithFilter: any = someArray.filter((product)=> product.size == "S").length;
