...
*/
const arrayOfNames: string[] = someArray.map((product)=> product.name);
const arrayOfPrices: number[] = someArray.map((product)=> product.price);
...
const resultArray: any[] = someArray.map((product)=> `${product.name}: ${(product.price/100).toFixed(2)}zł`);
console.log("Result:");
